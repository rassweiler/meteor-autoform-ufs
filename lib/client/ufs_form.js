...
Template.ufs_form.onCreated(function(){
	...
	this.UFS_ShowFiles = new ReactiveVar( false );
	this.UFS_UploadedFiles = new ReactiveVar( null );
...
Template.ufs_form.helpers({
	...
	showFiles:function(){
		return Template.instance().UFS_ShowFiles.get();
	},
	...
	allowReuse:function(){
		return UploadFS.config.allowReuse;
	}
});
...
Template.ufs_form.events({
	...
	'click [name=select]': function (event, template) {
		event.preventDefault();
		Template.instance().UFS_ShowFiles.set(true);
	},
	'click [name=ok]': function (event, template) {
		event.preventDefault();
		var files = UFS_Files.get();
		var filesU = Template.instance().UFS_UploadedFiles.get();
		if(filesU != null){
			if(files == null){
				files = [];
			}
			for(var i = 0; i < filesU.length; ++i){
				if(files.indexOf(filesU[i]) == -1){
					files.push(filesU[i]);
				}
			}
			UFS_Files.set(files);
			Template.instance().UFS_UploadedFiles.set(null);
		}
		Template.instance().UFS_ShowFiles.set(false);
	},
	'click [name=cancel]': function (event, template) {
		event.preventDefault();
		Template.instance().UFS_UploadedFiles.set(null);
		Template.instance().UFS_ShowFiles.set(false);
	},
	'click .file': function (event, template) {
		event.preventDefault();
		let id = event.target.id;
		if(id != null){
			let files = Template.instance().UFS_UploadedFiles.get();
			if(files == null){
				files = [];
			}
			...
			if (index > -1) {
				...
				Template.instance().UFS_UploadedFiles.set(files);
			}else{
				files.push(id);
				className += " selected";
				event.target.className = className;
